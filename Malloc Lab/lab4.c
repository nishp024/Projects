...
int main(int argc, char **argv)
{
    ...
    else if (dprms.UnitDriver == 2)
    {
        ...
        num_bytes_2 = (units_in_first_page/4)*unit_size;
        p2 = (int *) Mem_alloc(num_bytes_2);
        printf("second: %d bytes (%d units) at p=%p \n", 
                num_bytes_2, num_bytes_2/unit_size, p2);
        Mem_print();

        // allocate for 3rd pointer to 1/4 of a page
        ...
        // allocate for 4th pointer to 1/4 a page
        num_bytes_4 = (units_in_first_page/4)*unit_size;
        p4 = (int *) Mem_alloc(num_bytes_4);
        printf("fourth: %d bytes (%d units) at p=%p \n", 
                num_bytes_4, num_bytes_4/unit_size, p4);
        Mem_print();
        ...
        printf("first free of 1/4 a page p=%p \n", p3);
        Mem_free(p3);
        Mem_print();

        printf("second free of 1/4 a page p=%p \n", p1);
        Mem_free(p1);
        Mem_print();
    ...
    //   -- request more bytes than in two pages
